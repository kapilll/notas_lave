...
def get_trade_by_id(store: EventStore, trade_id: int) -> dict | None:
    """Reconstruct a single trade from its events."""
    conn = store._conn

    rows = conn.execute(
        "SELECT event_type, data FROM trade_events "
