...
### Added
- **Trade Autopsy** — after every trade closes, an AI-powered post-trade report is generated by Claude analyzing market context, entry quality, key levels, why the trade worked or failed, and actionable lessons. Reports saved to `TradeLog.lessons_learned` (DB column) and `data/autopsies/*.md` (markdown files).
- **Strategy disable via config** — `DISABLED_STRATEGIES` env var accepts a comma-separated list of strategy names to hard-exclude from the arena (e.g. `williams_system,trend_momentum,breakout_system`). Bypasses the trust/leaderboard system — disabled strategies never produce proposals.

### Changed
- Disabled `williams_system`, `trend_momentum`, `breakout_system` on the live VM (added to `DISABLED_STRATEGIES` in `.env`). Combined negative expectancy across 18 trades (-$27.78). `order_flow_system` kept active — 0 trades, needs data.
